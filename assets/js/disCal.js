...
function now_map() {
    map = new Tmapv2.Map("map_div", {
        center: new Tmapv2.LatLng(
            parseFloat(localStorage.getItem('coords_lat')),
            parseFloat(localStorage.getItem('coords_lon'))
        ),
        width: "100%",
        height: "400px",
        zoom: 13,
...
function default_map() {
    ...
    map.setCenter(new Tmapv2.LatLng(
        parseFloat(localStorage.getItem('receiver_latitude_y')),
        parseFloat(localStorage.getItem('receiver_longitude_x'))
    ));
    map.setZoom(12);
    ...
    marker_s = new Tmapv2.Marker({
        position: new Tmapv2.LatLng(
            parseFloat(localStorage.getItem('sender_latitude_y')),
            parseFloat(localStorage.getItem('sender_longitude_x'))
        ),
    ...
    // 3. 경로탐색 API 사용요청 $("#Delivery-fee-Calculation")      .click( function() { 기존
    // 맵에 있던 정보들 초기화
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var tmoney = " 예상 추가 배달비 : " + (
                (resultData[0].properties.totalDistance / 1000).toFixed(1) - maginot.value
            ) * 1000 + "원";
            if ((resultData[0].properties.totalDistance / 1000).toFixed(1) < maginot.value) {
                tmoney = maginot.value + "km 이내 입니다.";
            }
            console.log(maginot.value)
            var result = "<center> <p>" + tDistance + "<br><b>" + tmoney + "</b></p> </cent" +
                    "er>";
